...
export const calcPrices = (runners: Runner[]) => {
	...
	const averages = Object.entries(result).reduce(
		(acc: { [key in keyof CalculationObject]: number }, [key, values]) => {
...
export const calcParams = (data: RunnerCalcParams) => {
	...
	const matWeight = Math.ceil(
		Math.PI * ((Da + allowance) / 2) ** 2 * (Ba + allowance) * (matDensity / 1000000000),
	);
